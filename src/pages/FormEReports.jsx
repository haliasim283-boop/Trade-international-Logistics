...
import { FormEPaymentModal } from '../components/forme/FormEPaymentModal'
import { printFormEReport } from '../components/forme/FormEPrintView'
...
function exportCSV(shipments, supplier, dateFrom, dateTo) {
  const header = ['Date','AWB No.','Client','USD Value','PKR Rate','Form E Amount (PKR)']
  ...
  a.href     = url
  a.download = `form-e-${supplier?.name ?? 'report'}-${dateFrom}-${dateTo}.csv`
  a.click()
...
export default function FormEReports() {
  const location = useLocation()
  const range    = defaultRange()

  // Selectors
  const [suppliers,          setSuppliers]          = useState([])
  const [selectedSupplierId, setSelectedSupplierId] = useState(location.state?.supplierId ?? '')
  ...
  // ── Load suppliers + settings once ─────────────────────────────────────────
  useEffect(() => {
    if (!supabase) return
    supabase.from('form_e_suppliers').select('*').eq('is_active', true).order('name')
      .then(({ data }) => {
        const list = data ?? []
        setSuppliers(list)
  ...
  // ── Load data when supplier/dates change ────────────────────────────────────
  const loadData = useCallback(async () => {
    ...
    const [{ data: sData, error: sErr }, { data: pData }] = await Promise.all([
      ...
      supabase.from('shipments')
        .select('id,flight_date,awb_number,origin,destination,form_e_usd_value,form_e_pkr_rate,form_e_amount_pkr,clients(name)')
      ...
      // ALL payments to this supplier (no date filter — gives overall balance)
      supabase.from('form_e_payments')
        .select('*')
  ...
  async function deletePayment() {
    await supabase.from('form_e_payments').delete().eq('id', deletePayId)
    setDeletePayId(null); loadData()
  }

  // ── Print ───────────────────────────────────────────────────────────────────
  function handlePrint() {
    printFormEReport({ supplier, shipments, payments, summary, dateFrom, dateTo, settings })
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-navy">Form E Reports & Payables</h1>
          <p className="text-sm text-gray-500 mt-0.5">Track Form E transactions and amounts owed to suppliers</p>
        </div>
        {supplier && (
          <div className="flex gap-2">
            <Button variant="secondary" onClick={() => exportCSV(shipments, supplier, dateFrom, dateTo)}>
      ...
      {!selectedSupplierId ? (
        <div className="text-center py-20 text-gray-400">
          <p className="text-base">Select a Form E supplier above to view their report.</p>
        </div>
      ) : loading ? (
        ...
        <>
          ...
          <div className="bg-navy text-white rounded-xl px-6 py-4">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div>
                <p className="text-xs uppercase tracking-widest text-blue-200 mb-0.5">Form E Report</p>
              ...
              <div className="text-right">
                ...
                <p className="text-3xl font-bold font-mono">{shipments.length}</p>
              </div>
            </div>
          </div>
          ...
          <Card>
            <div className="px-4 pt-4 pb-2 flex items-center justify-between">
              <h3 className="font-semibold text-navy text-sm uppercase tracking-wide">
                Form E Transactions — {fmtDate(dateFrom)} to {fmtDate(dateTo)}
              </h3>
              <span className="text-xs text-gray-400">{shipments.length} record{shipments.length !== 1 ? 's' : ''}</span>
            </div>
              ...
              <table className="w-full text-sm border-collapse">
                <thead className="bg-navy text-white">
                  <tr>
                    ...
                    <th className="px-4 py-2.5 text-right text-xs font-semibold uppercase tracking-wide">PKR Rate</th>
          ...
          <Card>
            <div className="px-4 pt-4 pb-2 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">All payments ever made to {supplier?.name}</p>
              </div>
                  ...
                  <tfoot className="border-t-2 border-gray-300">
                    ...
                    <tr className={summary.balanceDue > 0 ? 'bg-red-50' : 'bg-green-50'}>
                      <td colSpan={2} className="py-2.5 font-bold text-gray-900">
                        Balance Due (Period Payable − All Paid)
                      </td>
      ...
      {/* ── Modals ── */}
      {showPaymentModal && selectedSupplierId && (
        <FormEPaymentModal
