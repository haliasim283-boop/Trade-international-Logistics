...
export function FormEPaymentModal({ supplierId, periodStart, periodEnd, existing, onClose, onSaved }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const { error } = isEdit
        ? await supabase.from('form_e_payments').update(payload).eq('id', existing.id)
        : await supabase.from('form_e_payments').insert({
  ...
  return (
    <Modal title={isEdit ? 'Edit Payment' : 'Record Payment to Form E Supplier'} onClose={onClose}>
