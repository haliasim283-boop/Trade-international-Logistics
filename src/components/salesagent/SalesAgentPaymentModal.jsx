...
export function SalesAgentPaymentModal({ agentId, periodStart, periodEnd, existing, onClose, onSaved }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const { error } = isEdit
        ? await supabase.from('sales_agent_payments').update(payload).eq('id', existing.id)
        : await supabase.from('sales_agent_payments').insert({
  ...
  return (
    <Modal title={isEdit ? 'Edit Payment' : 'Record Payment to Sales Agent'} onClose={onClose}>
