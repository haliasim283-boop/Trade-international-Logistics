...
export function ClearingPaymentModal({ agentId, periodStart, periodEnd, existing, onClose, onSaved }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const { error } = isEdit
        ? await supabase.from('clearing_agent_payments').update(payload).eq('id', existing.id)
        : await supabase.from('clearing_agent_payments').insert({
  ...
  return (
    <Modal title={isEdit ? 'Edit Payment' : 'Record Payment to Clearing Agent'} onClose={onClose}>
