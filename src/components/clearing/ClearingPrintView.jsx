...
export function printClearingReport({ agent, shipments, payments, summary, dateFrom, dateTo, settings }) {
  const rows = shipments.map((s) => `
    <tr>
      ...
      <td class="center">${esc(s.origin)}</td>
...
<head>
  <meta charset="utf-8"/>
  <title>Clearing Agent Report — ${esc(agent?.name)}</title>
  <style>
...
<body>
  ...
  <div class="header">
    ...
    <div class="header-right">
      ...
      <div class="sub">${esc(agent?.name)} — ${esc(agent?.city)} (${esc(agent?.origin_code)})</div>
      <div class="sub">Period: ${fmtDate(dateFrom)} &ndash; ${fmtDate(dateTo)}</div>
    </div>
  </div>

  <div class="meta">
    <span><strong>Agent:</strong> ${esc(agent?.name)}</span>
    <span><strong>City:</strong> ${esc(agent?.city)} (${esc(agent?.origin_code)})</span>
